...
import {
  ...
  useTalentPool,
  useTalentPools,
} from '@/hooks/business/useTalentPools';
import type { TalentPool, TalentPoolInput, TalentPoolMemberInput, TalentPoolType } from '@/hooks/business/types';
...
function poolLabel(type: string) {
  return POOL_TYPE_LABEL[type] || type;
}
  ...
  return (
    ...
    <Card className="cursor-pointer p-4 transition-colors hover:border-brand-200 dark:hover:border-brand-500/40">
      ...
      <div className="mt-3 flex items-center gap-2">
        <Badge tone="brand">{poolLabel(pool.pool_type)}</Badge>
        <span className="flex items-center gap-1 text-xs font-semibold text-ink-500 dark:text-ink-400">
          <Users className="h-3.5 w-3.5" /> {pool.member_count}
        </span>
      </div>
      {pool.tags.length > 0 && (
...
function AddMemberForm({ poolId }: { poolId: string }) {
  ...
  const [form, setForm] = useState<TalentPoolMemberInput>({ candidate_name: '' });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2 rounded-xl border border-ink-100 p-3 dark:border-ink-800">
      <div className="grid grid-cols-2 gap-2">
        ...
        <Input value={form.candidate_email || ''} onChange={(e) => setForm((p) => ({ ...p, candidate_email: e.target.value || undefined }))} placeholder="Email (optional)" />
      </div>
      <Input value={form.source || ''} onChange={(e) => setForm((p) => ({ ...p, source: e.target.value || undefined }))} placeholder="Source (optional)" />
      <textarea
        value={form.notes || ''}
...
function PoolDrawer({ poolId, onClose }: { poolId: string | null; onClose: () => void }) {
  const { data: pool, isLoading, isError, error } = useTalentPool(poolId || undefined);
        ...
        {pool && (
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-ink-900 dark:text-white">Members ({pool.members.length})</p>
              <AddMemberForm poolId={pool.id} />
            </div>

            {pool.members.length === 0 ? (
              <Card className="border-dashed py-10 text-center">
                <p className="text-sm text-ink-400 dark:text-ink-500">No members in this pool yet.</p>
              </Card>
            ) : (
...
function NewPoolModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [form, setForm] = useState<TalentPoolInput>({ name: '', pool_type: 'general', tags: [] });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setForm({ name: '', pool_type: 'general', tags: [] });
      setTagsInput('');
      onClose();
    } catch (e2) {
      setErr(getApiErrorMessage(e2));
  ...
  return (
    <Modal open={open} onClose={onClose} labelledBy="new-pool-title" className="max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-3 p-5">
        <Input data-autofocus value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="Pool name" required />
        <textarea
        ...
        />
        <div>
          <label className="mb-1 block text-xs font-semibold text-ink-500 dark:text-ink-400">Pool type</label>
          <select
            value={form.pool_type}
            onChange={(e) => setForm((p) => ({ ...p, pool_type: e.target.value as TalentPoolType }))}
          ...
          >
            {Object.entries(POOL_TYPE_LABEL).map(([value, label]) => (
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" loading={createPool.isPending} disabled={!form.name.trim()}>
            Create pool
          </Button>
        </div>
      </form>
...
export default function TalentPoolsPage() {
  const { data, isLoading, isError, error } = useTalentPools();
    ...
    <div className="mx-auto max-w-[1400px] space-y-5 px-4 py-5 lg:px-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-500 dark:text-ink-400">Business-owned candidate pools sourced, referred, or held back for future roles.</p>
        </div>
