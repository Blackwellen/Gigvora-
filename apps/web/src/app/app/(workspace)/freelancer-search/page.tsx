...
// General-purpose "find freelancers" browse page — reuses the same
// GET /public/talent query the projectId-scoped invite-to-project page
// uses, but stands alone: with no ?projectId=, results just link to the
// professional's profile; with a ?projectId= (e.g. reached by clicking
// through from a project), each result also gets an "Invite to project"
// action via the existing useInviteToProject hook.
function FreelancerSearchInner() {
  ...
  const { data: results, isLoading, isError, error } = useTalentSearch(query, countryFilterName);
  ...
  async function handleInvite(slug: string) {
    ...
    try {
      ...
      setInviteError(getApiErrorMessage(err, 'Could not send the invitation.'));
    }
  }
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-4 px-4 py-5 lg:px-6">
      <div>
        <h1 className="text-xl font-bold text-ink-900 dark:text-white">Find Freelancers</h1>
        <p className="mt-1 text-sm text-ink-500 dark:text-ink-400">
          {projectId ? 'Search professionals and invite them to your project.' : 'Search the professional directory by name, skill, or role.'}
        </p>
      </div>
      ...
      {(isError || inviteError) && (
        <p className="text-sm text-red-600 dark:text-red-400">{inviteError || getApiErrorMessage(error, 'Could not load professionals.')}</p>
      )}
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {(results || []).map((p) => (
          <Card key={p.id} className="p-4">
            <Link href={`/profile/${p.slug}`} className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {p.location && <p className="truncate text-xs text-ink-400 dark:text-ink-500">{p.location}</p>}
                {p.verified && (
