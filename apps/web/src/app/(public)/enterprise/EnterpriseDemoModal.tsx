...
// Local demo-request / contact-sales modal for the Enterprise page. Kept
// local (not imported from the for-businesses route) so concurrent page
// builds never share a mutable file.
export function EnterpriseDemoModal({ open, onClose, product = 'enterprise' }: { open: boolean; onClose: () => void; product?: string }) {
  ...
  return (
    <Modal open={open} onClose={handleClose} labelledBy="enterprise-demo-title" className="max-w-lg">
      <ModalHeader title="Contact enterprise sales" onClose={handleClose} />
        ...
        {status === 'success' ? (
          <div className="flex flex-col items-center py-6 text-center">
            ...
            <p className="mt-1 text-sm text-ink-500">
              Thanks, {form.name.split(' ')[0] || 'there'}. Our enterprise team will reach out within one business day.
            </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <Input data-autofocus required value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="Jane Smith" />
              </div>
              <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <Input value={form.jobTitle} onChange={(e) => update('jobTitle', e.target.value)} placeholder="VP of Engineering" />
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold text-ink-700">Tell us about your organization</label>
              <textarea
                rows={3}
                value={form.message}
            ...
            {(status === 'error' || (status === 'idle' && error)) && <p className="text-xs text-red-600">{error}</p>}
