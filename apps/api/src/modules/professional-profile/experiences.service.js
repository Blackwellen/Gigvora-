...
export async function list(userId) {
  ...
  const companies = companyIds.length ? await db('companies').whereIn('id', companyIds).select('id', 'name', 'logo_url') : [];
  const companyById = Object.fromEntries(companies.map((c) => [c.id, c]));
  return rows.map((r) => ({ ...r, company: r.company_id ? companyById[r.company_id] || null : null }));
}
...
export async function create(userId, input) {
  ...
  const { companyId, orgName } = await resolveCompany(input.orgName);
...
export async function update(userId, id, input) {
  ...
  if ('orgName' in input) {
    const resolved = await resolveCompany(input.orgName);
