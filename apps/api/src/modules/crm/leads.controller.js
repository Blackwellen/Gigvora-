...
export const convertHandler = asyncHandler(async (req, res) => {
  const owner = resolveOwner(req);
  const result = await service.convert(owner, req.user.sub, req.params.id, req.body);
  res.json({ data: result });
});

export const disqualifyHandler = asyncHandler(async (req, res) => {
  const owner = resolveOwner(req);
  const record = await service.disqualify(owner, req.user.sub, req.params.id, req.body);
  res.json({ data: record });
});
