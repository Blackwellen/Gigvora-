...
export const moveHandler = asyncHandler(async (req, res) => {
  const owner = resolveOwner(req);
  const record = await service.move(owner, req.user.sub, req.params.id, req.body);
  res.json({ data: record });
});

export const closeHandler = asyncHandler(async (req, res) => {
  const owner = resolveOwner(req);
  const record = await service.close(owner, req.user.sub, req.params.id, req.body);
  res.json({ data: record });
});
