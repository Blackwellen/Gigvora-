...
import * as service from './shortlists.service.js';
...
export const listHandler = asyncHandler(async (req, res) => {
  ...
  const result = await service.list(requireCompanyId(req), { job_id, status });
...
export const updateMemberHandler = asyncHandler(async (req, res) => {
  const record = await service.updateMember(req.params.id, req.params.memberId, requireCompanyId(req), req.body);
  res.json({ data: record });
});
